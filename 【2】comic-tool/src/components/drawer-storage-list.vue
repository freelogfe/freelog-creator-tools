        ...
        <el-popover :visible="data.uploadPopShow" width="fit-content" placement="bottom-start">
            ...
            <template v-if="data.bucketList.length">
              ...
              >
                ...
                <el-option
                  ...
                  value=""
                  @click.prevent="openBucketCreater()"
          ...
          <template #reference>
            <div class="upload-btn" @click.stop="data.uploadPopShow = true">
              ...
              <div class="btn-text">{{ I18n("cbformatter_import_uploadcb_btn") }}</div>
            </div>
          </template>
        </el-popover>
      </div>
      ...
      <el-scrollbar>
        <div class="list-area" v-infinite-scroll="getObjects" :infinite-scroll-immediate="false">
          ...
          <div class="list-item" v-for="item in data.objectList" :key="item.objectId">
            <StorageObject :data="item" @select="$emit('select', item)" />
          </div>
        </div>
      </el-scrollbar>
    </template>
...
const props = defineProps(["active", "from", "closeSubPopup"]);
