...
export const html2md = (htmlText: string) => {
  // 解决当内容为空时编辑器会自动初始化为 <p><br></p> 导致 markdown 内容改动的问题
  ...
  if (preContent.length) {
    ...
    // 清除 <pre>、</pre> 标签
    markdownText = markdownText.replace(/<pre\s*[^>]*?>/gi, "```").replace(/<\/pre>/gi, "[~wrap]```[~wrap]");
  }
  ...
  if (codeContent.length) {
    ...
    // 清除 <code>、</code> 标签
    markdownText = markdownText.replace(/<code\s*[^>]*?>/gi, "`").replace(/<\/code>/gi, "`");
  }
  ...
  resourceContent = markdownText.match(/<span\s*data-w-e-type="resource"[^>]*?>[^]*?<\/span>/gi) || [];
  // 标记原文本中资源内容
  markdownText = markdownText.replace(/<span\s*data-w-e-type="resource"[^>]*?>[^]*?<\/span>/gi, "`#resourceContent#`");
  ...
  // 引用：将 <blockquote> 转为 >
  markdownText = markdownText.replace(/<blockquote>/gi, "> ").replace(/<\/blockquote>/gi, "[~wrap][~wrap]");
  ...
  // 无序列表：将 <li>、<dd> 转为 -
  markdownText = markdownText.replace(/<li>|<dd>/gi, "- ").replace(/<\/li>|<\/dd>/gi, "[~wrap]");
  ...
  if (resourceContent.length) {
    for (let i = 0; i < resourceContent.length; i++) {
      ...
      // 资源类型
      const resourceTypeMatch = text.match(/data-resourceType="\[("|&#34;)(([^"][^&#34;])*)?("|&#34;)/i);
...
/** 匹配所有遍历处理 */
const matchAllStep = (iterator: IterableIterator<RegExpMatchArray>, markdownText: string): string => {
...
/** 获取匹配所有内容 */
const getMatchAllContent = (iterator: IterableIterator<RegExpMatchArray>, arr: string[], reg: RegExp) => {
  const step = iterator.next();
  if (step.done) return;
